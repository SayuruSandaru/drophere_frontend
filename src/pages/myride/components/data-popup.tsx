...
const DataPopup: React.FC<DataPopupProps> = ({ isOpen, onClose, data }) => {
    ...
    const formattedStartTime = data && data.ride ? format(new Date(data.ride.start_time), 'do MMM yy, h:mm a') : '';
    ...
    return (
        <>
            ...
            {data.status === 'ongoing' && (<MapPopup isOpen={isOpenMap} onClose={onCloseMap} data={data?.ride?.route} databasePath={`rides/R${data?.reservation_id}-I${data?.ride_id}`} />)}
        </>
    );
};
